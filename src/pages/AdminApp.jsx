...
export default function AdminApp() {
  ...
  function calcRosterGaps(schedule, slots, dates, staffList) {
    // Compare template slots vs actual assignments per dept
    const gaps = []
    // Track which staff are "claimed" per day (same logic as display)
    const claimed = {}
    // Process in DEPT_KEYS order
    ...
    DEPT_ORDER.forEach(deptKey => {
      slots.filter(s => s.dept === deptKey && s.is_recurring).forEach(slot => {
        ...
        const date = dates[di]
        // Count staff in this dept with this shift who aren't claimed yet
