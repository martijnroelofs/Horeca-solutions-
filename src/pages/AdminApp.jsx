...
function getWeeks() {
  ...
  for (let i = 0; i < 28; i++) {  // 28 weeks = ~7 months to cover next month safely
...
export default function AdminApp() {
  ...
  const autoGenPending = useRef(null) // localStorage-key als auto-generatie klaarstaat
  ...
  // Automatische maandgeneratie — draait vanaf de in Instellingen gekozen dag
  // (auto_generate_day, standaard 15, 0 = uitgeschakeld). Het effect wacht tot
  // alle data geladen is en ziet altijd de actuele state.
  useEffect(() => {
    if (!templateSlots.length || !allStaff.length) return // wacht op data
    if (settings.org_id === undefined) return // wacht tot instellingen geladen zijn
    if (generating || autoGenPending.current === 'running') return
    const genDay = settings.auto_generate_day ?? 15
    if (!genDay || genDay < 1) return // 0 of leeg = uitgeschakeld
    const now = new Date()
    if (now.getUTCDate() < genDay) return
    const nmYear = now.getUTCMonth() === 11 ? now.getUTCFullYear() + 1 : now.getUTCFullYear()
    const nmMonth = (now.getUTCMonth() + 1) % 12
    const nextMonthStr = `${nmYear}-${String(nmMonth + 1).padStart(2,'0')}`
    const storageKey = `autoGenerated_${orgId}_${nextMonthStr}`
    if (localStorage.getItem(storageKey)) return // deze maand al gedaan
    autoGenPending.current = 'running'
    show('🗓 Automatisch rooster genereren voor volgende maand...')
    handleGenerateMonth(true) // skipExisting: bestaande weken niet aanraken
      .then(() => localStorage.setItem(storageKey, 'true'))
      .finally(() => { autoGenPending.current = null })
  }, [templateSlots, allStaff, settings, generating])
  ...
  async function loadOvertime() {
    // Only count overtime within the current calendar year (1 jan – 31 dec)
    // Balance resets automatically on January 1st.
    const year = new Date().getUTCFullYear()
    const yearStart = `${year}-01-01`
    const yearEnd = `${year}-12-31`
    // Get rosters in this year
    const { data: yearRosters } = await supabase.from('rosters')
      .select('id').eq('org_id', orgId)
      .gte('week_start', yearStart).lte('week_start', yearEnd)
    const rosterIds = (yearRosters || []).map(r => r.id)
    if (!rosterIds.length) { setOvertimeLog({}); return }
  ...
  async function handleGenerateMonth(skipExisting = false) {
    ...
    const nextMonthNum = (now.getUTCMonth() + 1) % 12  // 0-indexed
    // First and last day of next month in YYYY-MM-DD format
    const firstDay = `${nextMonthYear}-${String(nextMonthNum + 1).padStart(2,'0')}-01`
    const lastDay = new Date(Date.UTC(nextMonthYear, nextMonthNum + 1, 0)).toISOString().slice(0, 10)
    // Find all weeks that have ANY day in the target month
    const monthWeeks = weeks.filter(w =>
      w.dates.some(d => d >= firstDay && d <= lastDay)
    )
    if (!monthWeeks.length) { show('Geen weken gevonden voor volgende maand'); return }
    const nextMonthLabel = new Date(Date.UTC(nextMonthYear, nextMonthNum, 1))
      .toLocaleDateString('nl-NL', { month:'long', year:'numeric', timeZone:'UTC' })

    setGenerating(true)
    show(`🪄 Genereer rooster voor ${nextMonthLabel}...`)

    // Carry overtime forward week-by-week: week 1's OT reduces week 2's hours, etc.
    // Overtime balance is per calendar year — reset if the target month is January.
    const targetYear = nextMonthYear
    let runningOT = nextMonthNum === 0 ? {} : { ...overtimeLog } // January (month 0) starts fresh
    ...
    const totalGaps = monthWeeks.reduce((sum, week) => {
      const sched = {}
      allStaff.forEach(s => { sched[s.id] = week.dates.map(d =>
        Object.values(assignments).flatMap(a => Object.entries(a))
          .find(([sid]) => sid === s.id)?.[1]?.[week.dates.indexOf(d)] || null
      )})
      return sum + calcRosterGaps(sched, templateSlots, week.dates, allStaff).reduce((a,g)=>a+g.missing,0)
    }, 0)
    const msg = totalGaps > 0
      ? `✓ ${nextMonthLabel} gegenereerd — ⚠️ ${totalGaps} diensten niet ingevuld`
      : `✓ Rooster gegenereerd voor ${monthWeeks.length} weken in ${nextMonthLabel}!`
    show(msg)
  }
  ...
  async function reviewSwap(id, status) {
    ...
    if (status === 'approved') {
      ...
      if (sw) {
        // week_end = week_start + 6 days (rosters table only has week_start)
        const roster = Object.values(rosters).find(r => {
          const weekEnd = new Date(r.week_start)
          weekEnd.setDate(weekEnd.getDate() + 6)
          const weekEndStr = weekEnd.toISOString().slice(0, 10)
          return sw.from_date >= r.week_start && sw.from_date <= weekEndStr
        })
        if (roster) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
      ...
      <Card style={{ padding:20 }}>
        ...
        {inp('Automatisch maandrooster genereren vanaf dag (1-28, 0 = uit)','auto_generate_day','number',{min:0,max:28,step:1})}
        <div style={{ color:C.inkMuted, fontSize:11, marginTop:-8, marginBottom:12 }}>
          Vanaf deze dag van de maand wordt het rooster voor de volgende maand automatisch
          aangemaakt zodra je de app opent. Bestaande en gepubliceerde weken blijven onaangetast.
        </div>
